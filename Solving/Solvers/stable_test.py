...
if __name__ == '__main__':
    ...
    h_candidates, v_candidates = find_revision_candidates(common_grid, full_grid, across, down,
                                                          threshold=thresh)
    while len(h_candidates) < 1 or len(v_candidates) < 1:
        thresh -= 0.15
        print(f"Threshold: {thresh}")
        h_candidates, v_candidates = find_revision_candidates(common_grid, full_grid, across, down,
                                                              threshold=thresh)
    previous_h = common_grid
    previous_v = common_grid
    ...
    while len(h_candidates) > 0 and len(v_candidates) > 0:
        ...
        previous_h = common_grid_h
        previous_v = common_grid_v
        ...
        if (thresh > 0.02):
            ...
            last_h_grid = common_grid_h
            last_v_grid = common_grid_v
            break
