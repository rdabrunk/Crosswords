...
if __name__ == '__main__':
    ...
    while len(h_candidates) > 0 or len(v_candidates) > 0:
        ...
        if (thresh > 0.15):
            ...
            break
    revised_grid_1 = revised_grid

    revised_grid = merged_grid
    thresh = 0.64

    h_candidates, v_candidates = find_revision_candidates(revised_grid, full_grid, across, down, threshold=thresh)
    print("Starting revision 2:")
    while len(h_candidates) > 0 or len(v_candidates) > 0:
        if len(h_candidates) > 0:
            revised_grid = revise_horizontal(revised_grid, h_candidates)
            h_candidates, v_candidates = find_revision_candidates(revised_grid, full_grid, across, down,
        ...
        if (thresh > 0.15):
            ...
            break
    revised_grid_2 = revised_grid

    # save_grid(revised_grid, f"{file_name}_revised")
    final_grid = get_common_grid(revised_grid_1, revised_grid_2)
    acc = analyze(final_grid, full_grid)

    thresh = 0.64
    h_candidates, v_candidates = find_revision_candidates(final_grid, full_grid, across, down, threshold=thresh)
    print("Starting final revision 1")
    while len(h_candidates) > 0 or len(v_candidates) > 0:
        if len(h_candidates) > 0:
            final_grid_1 = revise_horizontal(final_grid, h_candidates)
            h_candidates, v_candidates = find_revision_candidates(final_grid, full_grid, across, down,
                                                                  threshold=thresh)
            # thresh += 0.03

        if len(v_candidates) > 0:
            final_grid = revise_vertical(final_grid, v_candidates)
            h_candidates, v_candidates = find_revision_candidates(final_grid, full_grid, across, down,
                                                                  threshold=thresh)
            # thresh += 0.03
        ...
        if (thresh > 0.15):
            ...
            break
    final_grid_1 = final_grid

    thresh = 0.64
    h_candidates, v_candidates = find_revision_candidates(final_grid, full_grid, across, down, threshold=thresh)
    print("Starting final revision 2")
    while len(h_candidates) > 0 or len(v_candidates) > 0:
        if len(h_candidates) > 0:
            final_grid = revise_horizontal(final_grid, h_candidates)
            h_candidates, v_candidates = find_revision_candidates(final_grid, full_grid, across, down,
                                                                  threshold=thresh)
            # thresh += 0.03
        ...
        if (thresh > 0.15):
            ...
            break
    final_grid_2 = final_grid

    final_grid = get_common_grid(final_grid_1, final_grid_2)
